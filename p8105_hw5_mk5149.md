...
Now, we create our simulation grid. This code is identical to the
previous answer. It runs our new function run_one_t_test for each
true_mu 5000 times.
...
We group our results by `true_mu` and calculate the proportion of
rejections (`p.value < 0.05`).
...
- Why it happens: The plot shows that the average $\hat{\mu}$ from all
  simulations (the blue line) falls perfectly on the $y=x$ dashed line.
  This confirms that the sample mean $\hat{\mu}$ is an unbiased
  estimator of the true $\mu$.
- However, the red line (Rejected simulations only) is biased high. To
